...
def test(test_loader,model):
    ...
    sigmoid = nn.Sigmoid()
    with torch.no_grad():
        for i, (x_batch,x_meta) in enumerate(test_loader):
            y_pred = model(x_batch, x_meta)
...
def main():
    ...
    train_X, test_X, train_y, word_index, train_meta, test_meta= utils.load_and_prec(config)
    ...
    for fold, (train_idx, valid_idx) in enumerate(splits):
        ...
        train_dataset = QuoraDataset(mode='train', x_meta=meta_train_fold, x_fold=x_train_fold, y_fold=y_train_fold )
        valid_dataset = QuoraDataset(mode='train', x_meta=meta_val_fold, x_fold=x_val_fold, y_fold=y_val_fold)
